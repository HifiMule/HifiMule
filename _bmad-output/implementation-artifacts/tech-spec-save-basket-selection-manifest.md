...
files_to_modify: ['hifimule-daemon/src/device/mod.rs', 'hifimule-daemon/src/rpc.rs', 'hifimule-ui/src/state/basket.ts', 'hifimule-ui/src/components/BasketSidebar.ts']
...
When a device is connected, the selection of artists and playlists (the "basket") is not restored from previous syncs. Because the `.hifimule.json` manifest only stores the actual synchronized files and not the high-level user selection, the user loses context of what they had originally selected to synchronize.

### Solution

Store the selected artists, albums, or playlists (the basket contents) directly in the `.hifimule.json` manifest on the device. When the device is loaded, the `hifimule-ui` will read this selection from the manifest and pre-populate the basket. This ensures that if the server-side contents of a playlist or artist change, the next sync will automatically detect and synchronize the new items under those existing selections.

### Scope

**In Scope:**
- Update `.hifimule.json` schema to include a `basket` or `selections` field.
- Update `hifimule-daemon` to read and write these selections during the sync or manifest creation process.
- Update `hifimule-ui` (e.g., `basket.ts`, `rpc`) to send the user's basket items to the daemon when syncing.
- Update `hifimule-ui` to restore the basket contents from the manifest when a device is connected.
...
| File | Purpose |
| ---- | ------- |
| `hifimule-daemon/src/device/mod.rs` | Defines `DeviceManifest`. This is where the `.hifimule.json` schema is defined. |
| `hifimule-daemon/src/rpc.rs` | Exposes RPC endpoints. We need to expose or return the basket state to the UI upon device connection or state fetch. |
| `hifimule-daemon/src/sync.rs` | Handles manifest updates during sync. We could save the basket state at the beginning or end of `sync_execute`. |
| `hifimule-ui/src/state/basket.ts` | The UI singleton for the basket. Needs to load data from the daemon side. |
| `hifimule-ui/src/components/BasketSidebar.ts` | Triggers the sync and could trigger saving the basket to the daemon. |
...
- [x] Task 4: Add `manifest_save_basket` RPC method
  - File: `hifimule-daemon/src/rpc.rs`, `hifimule-daemon/src/device/mod.rs`
...
- [x] AC 1: Given a device with a populated basket in its `.hifimule.json`, when the device is connected, then the UI basket is populated with those items.
- [x] AC 2: Given a connected device, when the user adds or removes items from the UI basket, then the `.hifimule.json` manifest is updated with the new basket contents.
- [x] AC 3: Given an existing `.hifimule.json` without a `basket_items` field, when the daemon reads it, then it parses successfully with an empty basket.
...
- **Daemon Unit Tests**: Add test in `hifimule-daemon/src/device/tests.rs` to verify that `DeviceManifest` serialization/deserialization works with and without `basket_items`.
- **Manual Verification**:
  ...
  5. Check `.hifimule.json` on the device manually to verify it contains the basket payload.
