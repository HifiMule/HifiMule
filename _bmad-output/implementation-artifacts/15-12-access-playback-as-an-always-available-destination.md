...
Review date: 2026-09-19. Scope: `bf8f16f..912af9f` (full story including follow-up fixes). All three review layers completed: Blind Hunter, Edge Case Hunter, Acceptance Auditor. Triaged result: 0 decision-needed, 15 patch, 1 defer, 0 dismissed. Duplicate reports are merged below.

Validation: the focused destination/playback UI suites passed all 38 tests. Additional temporary VM checks against production TypeScript reproduced (a) two metadata fetches for the same page after only a state-sequence update, (b) replacement of the paging button node, and (c) a paused session remaining on a recoverable error after a successful unchanged snapshot refresh. Daemon and installed hardware/platform suites were not rerun during this review; existing recorded results are not new review evidence.

- [x] [Review][Patch] R1 [P1] Correct camelCase fields on destination enum variants [hifimule-daemon/src/device/mod.rs:519] — Enum-level `rename_all` renames variants, not their fields: the daemon emits `pending_id`/`device_id`, while the UI expects `pendingId`/`deviceId`. Pending destinations consequently collide under `pending:undefined` and Setup lacks its required identity. Apply field renaming to both the response enum and the strict `Selection::PendingDevice` decoder in `rpc.rs:7900`, which rejects the declared camelCase request. Add actual serialized-wire coverage. AC1/4. Sources: blind+edge.
- [x] [Review][Patch] R2 [P1] Flush the outgoing physical basket before selecting Playback [hifimule-ui/src/components/DestinationHub.ts:101] — Flush currently depends on the target being a device. Editing A then choosing Playback within the one-second save debounce clears the daemon physical target before persistence; the queued save fails or the sidebar clears the local items first. Flush the outgoing managed destination before changing selection and preserve selection if saving fails. AC2 and closed gate 1. Sources: blind+edge+auditor.
- [x] [Review][Patch] R3 [P1] Reserve pending initialization atomically and fence completion [hifimule-daemon/src/device/mod.rs:1437] — Identity/revision validation only takes a read lock, then releases it for device I/O. Two requests can validate the same pending entry and write different generated device IDs concurrently. Completion at line 1605 also inserts the device without rechecking that the pending entry is still present, so removal during I/O can publish stale connected state. Reserve the exact entry exclusively, handle failure recovery, and validate its lifetime before publishing success. The RPC mutation guard counts concurrent operations; it does not serialize them. AC4 target-safe Setup. Sources: edge+blind.
- [x] [Review][Patch] R4 [P2] Remove failed MSC discovery state when the mount disappears [hifimule-daemon/src/device/mod.rs:2326] — A failed probe removes its mount from `known_mounts` at line 2383, but disappearance detection iterates only that set. Unplugging a failed mount emits no removal and leaves its discovery issue and retry entry indefinitely. Include failed-but-present identities in disappearance cleanup. AC4. Sources: blind+edge.
- [x] [Review][Patch] R5 [P2] Preserve the original arrival token across discovery retries [hifimule-daemon/src/device/mod.rs:2348] — MSC and MTP allocate a fresh mutation token on every retry (also line 2509). An arrival that fails, followed by an explicit Playback choice, can later recover with a newer token and steal selection. Store the initial observation token for the entire physical presence lifetime; only genuine re-arrival gets a new one. AC4 total mutation order. Sources: blind+edge.
- [x] [Review][Patch] R6 [P2] Share the occurrence metadata concurrency limiter across requests [hifimule-daemon/src/rpc.rs:839] — A new eight-permit semaphore is created per request, so overlapping requests or clients each admit eight provider lookups. Move the limiter to shared state and verify aggregate concurrency across simultaneous requests. Explicit occurrence-display global maximum of eight. Sources: blind+auditor.
- [x] [Review][Patch] R7 [P2] Reuse queue-page metadata across transport-only updates [hifimule-ui/src/components/PlaybackDestination.ts:39] — Every accepted state sequence calls `loadPage()` even when queue identity and cursor are unchanged. Playback progress therefore repeatedly lists and describes up to 100 tracks and can overlap unfinished requests. Cache the bounded visible page by queue identity/cursor and update the active transport/Preview presentation independently. The temporary reproduction observed two metadata calls for the identical page after one sequence-only update. AC3/7. Sources: blind+edge.
- [x] [Review][Patch] R8 [P2] Fence asynchronous queue responses by page request [hifimule-ui/src/components/PlaybackDestination.ts:52] — Response checks compare only instance/session/queue revision. A slow old-page request can finish after Next and replace the new rows and `nextCursor` while cursor/history still refer to the newer page. Capture a request generation and page cursor, reject superseded successes/errors, and prevent duplicate navigation while a request is pending. AC3/7. Sources: blind+edge+auditor.
- [x] [Review][Patch] R9 [P2] Retry queue loading even when the refreshed session is unchanged [hifimule-ui/src/components/PlaybackDestination.ts:61] — After a transient page failure the component refreshes the shared store, but equal state sequences are suppressed by `PlaybackStore.accept`. A paused session consequently stays on the error indefinitely with no retry action. Explicitly restart the page with bounded retry or expose an actionable retry independently of subscriber notification. Reproduced with production components: one failed page call, successful subsequent refresh, error still visible. AC7. Sources: edge+auditor.
- [x] [Review][Patch] R10 [P2] Preserve pagination controls and keyboard focus [hifimule-ui/src/components/PlaybackDestination.ts:85] — Every render creates fresh Previous/Next buttons and replaces the body at line 107. Activating Next or receiving a progress update removes the focused node. Keep paging controls mounted and update their state; verify focus after page navigation and transport updates. Node replacement was reproduced against production code. AC8. Sources: blind+edge.
- [x] [Review][Patch] R11 [P2] Restore output-inventory refresh independently of snapshot changes [hifimule-ui/src/components/PlaybackControls.ts:124] — The previous poll always refreshed the cached output inventory. The replacement only performs periodic refresh in the unchanged-sequence branch, but the shared store filters those snapshots before delivering them. Other snapshots refresh inventory only for a new instance. Background discovery completion/hotplug can therefore remain invisible until manual interaction. Provide a shared heartbeat/inventory update path and cover the real store-to-controls integration. AC2/7 preservation of output controls. Source: blind.
- [x] [Review][Patch] R12 [P2] Keep exactly one destination polling timer [hifimule-ui/src/components/DestinationHub.ts:40] — Every explicit selection calls `refresh()` while an existing timer remains scheduled; each call then schedules another self-renewing loop. Repeated navigation multiplies `getDaemonState` requests and increases stale-response exposure. Separate fetch-once from scheduling, cancel/reuse the timer, and fence obsolete results. Source: blind+edge.
- [x] [Review][Patch] R13 [P2] Announce discovery failures only when their revision changes [hifimule-ui/src/components/DestinationHub.ts:78] — Each poll destroys and recreates every issue with `role="status"`, even when the issue revision is unchanged. Stable failures are repeatedly inserted into live regions. Keep keyed issue nodes and change/announce text only when the corresponding revision changes. AC4/8 deduplicated accessible failure announcements. Source: blind.
- [x] [Review][Patch] R14 [P2] Publish sync operations with their target identity atomically [hifimule-daemon/src/sync.rs:1019] — `create_operation_for_device` first inserts a Running operation with `device_id: None`, awaits cancellation-token setup, then patches in the target. The concurrent removal handler matches only operations already carrying the removed device ID, so removal during this admission gap misses the operation and is never replayed. Insert target identity with the initial operation and coordinate removal/admission so target removal cannot escape cancellation/failure handling. AC4 operation pinning. Source: blind.
- [x] [Review][Patch] R15 [P2] Update and recover source labels after asynchronous loading [hifimule-ui/src/components/PlaybackDestination.ts:31] — `loadLabels()` changes the map without updating rendered rows or retrying a failed lookup. If a paused queue finishes rendering before `serverList`, it continues showing “source unavailable” despite successful label loading; transient label failures also persist until recreation. Rerender cached rows on label completion and offer bounded recovery without re-fetching track metadata. AC3/7. Source: auditor.
- [x] [Review][Defer] R16 [P2] Enforce physical-target checks on all existing basket entry points [hifimule-ui/src/components/TracksBrowseView.ts:580] — deferred, pre-existing. Per-track basket buttons only check the selected server and omit the `.basket-toggle-btn` CSS lock class; other CSS-only locks do not guard keyboard activation or store mutations. These paths already existed at the baseline. They remain an unmet AC2/closed-gate requirement: with no physical target, browsing/Play/Preview should work while physical basket mutation is disabled. Add a shared physical-selection guard in a follow-up; this review has not fixed or accepted the gap. Source: auditor.


### Review Patch Completion

2026-09-19: Applied all 15 authorized patch findings. R16 remains deferred and untouched. Story status is **in-progress**, because that pre-existing medium-severity physical basket guard gap still leaves AC2/closed gate 1 unmet; this is not a full acceptance sign-off.

- R1–R3: corrected serialized destination fields; flush outgoing managed baskets before navigation; reserve each pending Setup exclusively, cancel its work on removal/replacement, release failed reservations for retry, and reject stale completion.
- R4–R6: track failed device presence independently of successful probes, retain arrival tokens across retries, renew them after MTP storage removal, and enforce one process-wide eight-lookup metadata budget.
- R7–R11/R15: keep one bounded queue page and stable paging controls, discard obsolete page results, retry paused-session failures with backoff and a manual action, allow explicit retry of partial offline metadata, update late/retried source labels, and refresh output inventory through the shared store heartbeat.
- R12–R14: retain one destination poll timer, keep unchanged discovery issue nodes mounted, and publish sync target/cancellation state atomically while coordinating admission with device removal.

Validation: full controlled daemon suite **1,000 passed / 6 ignored**; focused production-component UI suite **48 passed**; installed-evidence validator **33 passed**; production UI build and `git diff --check` passed. The initial restricted full-daemon run failed because mock HTTP servers could not bind local sockets; the authorized rerun passed. Added regressions cover serialized pending selection, duplicate/cancelled/retried initialization, retry/removal observation order, sync admission visibility, concurrent metadata requests, queue recovery/focus/cache behavior, outgoing basket-save failure, poll/issue stability, and real shared-store output discovery. No installed hardware or additional OS evidence is claimed. No commit was created.
...
- 2026-09-19: Applied all 15 code-review patches and added regression coverage; retained deferred R16 and marked Story 15.12 in-progress pending its physical basket guard acceptance gap.
