...
3. **Resume the existing session from the desktop menu.** Given an existing paused session with a playable current track and usable selected output, Resume continues it without opening the main window or creating Radio. The first-launch bootstrap may supply that concrete saved endpoint but must not itself resume. Missing source/output or restoration failures remain paused and are reported accessibly. No resumable session means the action is unavailable or clearly explained.
...
5. **Reject unsupported or unsafe actions.** An unsupported command or Play while the selected output is unavailable neither fabricates success nor silently reroutes output. A later system-default change, partial discovery, absent saved endpoint, explicit null output configuration or invalid configuration cannot create a new native playback destination. Disable or omit unsupported native actions wherever the platform permits.
...
| `hifimule-daemon/src/playback/audio.rs`, `session/output_selection.rs` | Reuse existing resume/start/gating and exact selected-output policy. A missing first-run configuration may durably choose one complete-discovery concrete default without opening audio; thereafter preserve stable identity independent of display name, active ownership until close ACK, current-generation terminal cleanup and no default fallback. Read fully before any additional change. |
...
| Resume integration | Existing pipeline resume and source reopen at nonzero saved position; missing provider/auth/timeout; restored session; fresh first-run concrete-default bootstrap; no output, lost output, explicit-null/invalid config and pending switch. Assert bootstrap persists no audio effect, later default changes never substitute, no UI launch/new Radio, and no duplicated provider start. |
...
- 2026-09-16: Aligned the native-controls specification with first-launch output bootstrap: one complete-discovery concrete OS default may become the durable selected output without opening audio; Resume remains explicit and all later default, partial-discovery, missing, explicit-null and invalid-config cases retain no-substitution behavior.
