...
2. **Secure Credential Storage**:
   ...
   - Service Name: `jellysync` (internal reference)
...
- **Keyring**: Use `keyring::Entry::new("jellysync", "server_url")` pattern.
...
- New code should live in `jellysync-daemon/src/api` or directly in `main.rs` if small.
...
**New Files:**
- `jellysync-daemon/src/api.rs` - Jellyfin API client and file-based credential manager
- `jellysync-daemon/src/rpc.rs` - JSON-RPC 2.0 server implementation

**Modified Files:**
- `jellysync-daemon/src/main.rs` - Added RPC server startup in tokio runtime
- `jellysync-daemon/src/tests.rs` - Added credential storage tests
- `jellysync-daemon/Cargo.toml` - Added dependencies: reqwest, serde, serde_json, axum, lazy_static, mockito (dev)
