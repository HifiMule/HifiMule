...
1. **Dirty Flag Detection**: When a device is reconnected after an interrupted sync, the engine MUST detect the `dirty: true` flag in the `.hifimule.json` manifest. (AC: #1)
...
- [x] **T1: Add `dirty` and `pending_item_ids` fields to `DeviceManifest`** (AC: #1, #4, #6)
  - [x] T1.1: In `hifimule-daemon/src/device/mod.rs`, add two fields to `DeviceManifest` (after `synced_items`):
    ```rust
    #[serde(default)]
    pub dirty: bool,
    #[serde(default)]
  ...
  - [x] T1.2: Add `dirty: false, pending_item_ids: vec![],` to every `DeviceManifest { ... }` struct literal in the codebase (compilation fails otherwise):
    - `hifimule-daemon/src/device/tests.rs`: `test_write_manifest_creates_files` (~line 196), `test_write_manifest_overwrites_existing` (two constructions, ~line 237 and ~line 246)
    - `hifimule-daemon/src/sync.rs`: `empty_manifest()` test helper (~line 752)
    - `hifimule-daemon/src/rpc.rs`: `test_rpc_sync_get_device_status_map_with_synced_items` (~line 1218), `test_rpc_sync_calculate_delta_partial_failure` (~line 1316)

- [x] **T2: Add `cleanup_tmp_files` async function** (AC: #3)
  - [x] T2.1: In `hifimule-daemon/src/device/mod.rs`, add after `write_manifest`:
    ```rust
    ...
    ```
    **Implementation note**: Uses an iterative stack (no async recursion) to avoid Rust's boxed-future requirement for recursive async functions. Targets only `Music/` because that is the hardcoded managed path used by `execute_sync`. The `.hifimule.json.tmp` at device root is NOT in `Music/`, so it is unaffected.

- [x] **T3: Modify `execute_sync` for per-file manifest updates** (AC: #5)
  - [x] T3.1: Add `device_manager: Arc<crate::device::DeviceManager>` as the final parameter to `execute_sync` in `hifimule-daemon/src/sync.rs`. New signature:
    ```rust
...
- [x] **T7: Tests** (AC: #1–#6)
  ...
  - [x] T7.2 (`device/tests.rs`): `test_dirty_manifest_roundtrip` — write manifest with `dirty: true, pending_item_ids: ["id-1", "id-2"]`, read back, verify fields preserved exactly
    ...
    async fn test_dirty_manifest_roundtrip() {
        ...
        let content = tokio::fs::read_to_string(dir.path().join(".hifimule.json")).await.unwrap();
...
- **Atomic Manifest Writes** (from `architecture.md`, Safety & Atomicity): Every `write_manifest` call uses the existing Write-Temp-Rename pattern in `device/mod.rs`. Do NOT bypass this — never write `.hifimule.json` directly, always go through `write_manifest`. Per-file manifest updates use this same function.
...
**Files to Modify:**
1. `hifimule-daemon/src/device/mod.rs` — `DeviceManifest` struct extension, `cleanup_tmp_files` function
2. `hifimule-daemon/src/device/tests.rs` — struct literal updates + new tests (T7.1–T7.7)
3. `hifimule-daemon/src/sync.rs` — `execute_sync` signature change + per-file manifest updates + struct literal update in `empty_manifest()`
4. `hifimule-daemon/src/rpc.rs` — `handle_sync_execute` refactor + `handle_get_daemon_state` update + `handle_sync_get_resume_state` (new) + route table + struct literal updates + new tests (T7.8–T7.12)

**Files NOT to Modify:**
- `hifimule-daemon/src/api.rs` — No changes
- `hifimule-daemon/src/db.rs` — No changes (no SQLite needed for this story)
- `hifimule-daemon/Cargo.toml` — No new dependencies
...
7. **`cleanup_tmp_files` only scans `Music/`** — Do NOT scan the device root. The `.hifimule.json.tmp` at device root is managed by `write_manifest` itself and must not be deleted by cleanup.
...
- [hifimule-daemon/src/device/mod.rs:24-33](../../hifimule-daemon/src/device/mod.rs#L24) — `DeviceManifest` struct (add dirty + pending_item_ids after line 33)
- [hifimule-daemon/src/device/mod.rs:35-52](../../hifimule-daemon/src/device/mod.rs#L35) — `write_manifest` (add `cleanup_tmp_files` after this function)
- [hifimule-daemon/src/sync.rs:333-541](../../hifimule-daemon/src/sync.rs#L333) — `execute_sync` function (add device_manager param, per-file writes)
- [hifimule-daemon/src/sync.rs:441-477](../../hifimule-daemon/src/sync.rs#L441) — Successful add block in `execute_sync` (add per-file write after `synced_items.push`)
- [hifimule-daemon/src/sync.rs:494-512](../../hifimule-daemon/src/sync.rs#L494) — Successful delete block in `execute_sync` (add per-file write after `files_completed += 1`)
- [hifimule-daemon/src/sync.rs:515-538](../../hifimule-daemon/src/sync.rs#L515) — ID change block in `execute_sync` (add per-file write after `synced_items.push`)
- [hifimule-daemon/src/sync.rs:751-759](../../hifimule-daemon/src/sync.rs#L751) — `empty_manifest()` test helper (add dirty + pending_item_ids)
- [hifimule-daemon/src/rpc.rs:107-134](../../hifimule-daemon/src/rpc.rs#L107) — `handler` match (add `sync_get_resume_state` route)
- [hifimule-daemon/src/rpc.rs:319-334](../../hifimule-daemon/src/rpc.rs#L319) — `handle_get_daemon_state` (add `dirtyManifest` field)
- [hifimule-daemon/src/rpc.rs:725-847](../../hifimule-daemon/src/rpc.rs#L725) — `handle_sync_execute` (full refactor for T4)
- [hifimule-daemon/src/rpc.rs:792-815](../../hifimule-daemon/src/rpc.rs#L792) — Existing manifest extend/retain block INSIDE `tokio::spawn` (delete entirely, replace with dirty-clear)
...
No blocking issues. One additional struct literal was found in `hifimule-daemon/src/tests.rs` (crate integration test file, not listed in story's Dev Notes) — added `dirty: false, pending_item_ids: vec![]` to fix compilation.
