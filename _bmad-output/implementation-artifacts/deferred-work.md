...
- **Dirty-marker unit test does not assert 1-byte sentinel content** — `mtp_dirty_marker_detected_on_reconnect` checks presence and call order but not that the sentinel data is `b"\x00"`. A regression back to `b""` would not be caught. Add a data-content assertion in the mock test. [`device_io.rs`, `mtp_dirty_marker_detected_on_reconnect`]
