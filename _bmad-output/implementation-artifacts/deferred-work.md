...
- **Preview command deduplication is asymmetric across mutation families** (`hifimule-daemon/src/playback/session.rs:1206`) — Preview rejects command IDs already used by other mutations, but the other mutation handlers do not all reject IDs already present in `preview_dedup`. Apply the global `COMMAND_ID_REUSED` rule symmetrically.
- **Preview seek and native controls still use canonical-main identity/position** (`hifimule-daemon/src/playback/session.rs:2802`, `hifimule-daemon/src/playback/session.rs:2913`) — direct seek compares against the main occurrence, while native controls construct commands and relative targets from main occurrence/position. Active previews therefore reject these controls, and standalone previews report no current track. Route both through the existing active-transport helpers and add native-preview regressions.
- **Output-preference save failure does not project a paused Preview state** (`hifimule-daemon/src/playback/session/output_selection.rs:438`) — the gate closes but only canonical main state is paused, so a Preview snapshot can remain Buffering while no audio is authorized. Pause the active transport and expose the existing recoverable output error coherently.
- **Natural Preview completion swallows terminal persistence failure** (`hifimule-daemon/src/playback/session.rs:1032`) — `if let Ok(...)` ignores `finish_preview` failure, potentially leaving a completed audition projected as active without audio or an actionable persistence error. Preserve the active record and publish a recoverable persistence failure.
- **Story 15.11 installed/browser evidence remains incomplete** (`docs/playback-installed-test-checklist.md`) — no Windows/Linux/macOS installed Preview matrix has run, and current three-surface browser coverage is source-wiring inspection rather than behavioral selection/focus validation. Keep these rows open until real platform and browser interaction evidence exists.

## Deferred from: provider-aware `get_credentials` review (2026-09-18)

- **Database selection is not schema-unique or transactional** (`hifimule-daemon/src/db.rs:684-711`) — `get_server_config()` uses `WHERE selected = 1 LIMIT 1`, while `set_selected()` clears all rows before setting the target without a transaction. Corruption, manual writes, or a failed target update can therefore yield multiple or zero selected rows. The credential fix consistently uses the row returned by the existing selection contract, but hardening that contract requires a separate database migration/selection workflow.
