...
files_to_modify: ['jellysync-daemon/src/api.rs']
...
Add `"MusicArtist"` to the `CONTAINER_TYPES` constant in `jellysync-daemon/src/api.rs`. The existing container branch in `get_single_item_size()` already calls `get_child_items_with_sizes()` with `Recursive=true`, which traverses Artist → Albums → Tracks in a single Jellyfin API call. No structural changes are needed beyond adding the type string.

### Scope

**In Scope:**
- Size calculation for `MusicArtist` items in `jellysync-daemon/src/api.rs`
...
- **Container dispatch via `CONTAINER_TYPES` constant** ([api.rs:8](jellysync-daemon/src/api.rs#L8)): A `&[&str]` slice checked with `.contains()` determines whether an item is fetched recursively (container) or read directly (leaf). This is the single authoritative branching point for size calculation.
- **`get_child_items_with_sizes()` already uses `Recursive=true`** ([api.rs:378](jellysync-daemon/src/api.rs#L378)): The Jellyfin API call is `GET /Users/{uid}/Items?ParentId={id}&IncludeItemTypes=Audio,MusicVideo&Fields=MediaSources&Recursive=true`. The `Recursive=true` parameter makes Jellyfin flatten the Artist → Albums → Tracks hierarchy automatically, returning all leaf audio items. No extra recursion is needed in Rust.
- **Tests use `mockito` with `Server::new_async()`**: Container tests mock two endpoints — the item-details call (`/Users/{uid}/Items/{id}?Fields=MediaSources`) and the children call (`/Users/{uid}/Items?ParentId=...&Recursive=true`). See `test_get_item_sizes_album_container` ([api.rs:1093](jellysync-daemon/src/api.rs#L1093)) as the direct template.
...
| File | Purpose |
| ---- | ------- |
| [jellysync-daemon/src/api.rs](jellysync-daemon/src/api.rs) | **Primary change target.** `CONTAINER_TYPES` (line 8), `get_single_item_size()` (line 429), `get_child_items_with_sizes()` (line 361), existing tests (line 1067+) |
| [jellysync-daemon/src/rpc.rs](jellysync-daemon/src/rpc.rs) | RPC handler `handle_jellyfin_get_item_sizes()` (line 561) — read-only reference, no changes needed |
| [jellysync-ui/src/state/basket.ts](jellysync-ui/src/state/basket.ts) | `getTotalSizeBytes()` — read-only reference, no changes needed |
...
- [x] Task 1: Add `"MusicArtist"` to `CONTAINER_TYPES`
  - File: `jellysync-daemon/src/api.rs`
...
- [x] Task 2: Add unit test `test_get_item_sizes_artist_container`
  - File: `jellysync-daemon/src/api.rs` (inside `#[cfg(test)] mod tests`, after `test_get_item_sizes_album_container`)
