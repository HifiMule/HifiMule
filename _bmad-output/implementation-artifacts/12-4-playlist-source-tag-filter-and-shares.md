...
- [x] **Add the async materialization + run_pipeline layer** (`hifimule-daemon/src/auto_fill/` — new `fetch.rs` module, declared `pub mod fetch;` + re-export in `auto_fill/mod.rs`) (AC: 1, 2, 3, 4, 5, 7)
  - [x] Add `pub async fn expand_with_pipeline(provider: Arc<dyn MediaProvider>, pipeline: &AutoFillPipeline, params: AutoFillParams) -> Result<Vec<AutoFillItem>>`.
  - [x] **Materialize one pool per distinct `SourceKey`** referenced by `pipeline.sources` ∪ `pipeline.fallback` (dedupe identical `(kind, ref)` so a source used twice fetches once):
...
- [x] **Wire `provider_calculate_delta` single-server block** (`rpc.rs:2586-2671`) (AC: 1, 6, 8)
  - [x] Resolve the selected portable id (the same source `get_daemon_state`/12.2 uses: `state.db.get_server_config()?.server_id`, or the already-resolved `current_server_portable_id`), read `manifest.auto_fill.pipeline_for(selected_portable)`, and call `expand_auto_fill_slot(...)` instead of the direct `run_auto_fill_provider`. Preserve the existing budget derivation and dedup loop byte-for-byte for the default case.

- [x] **Force provider routing when a configured pipeline applies (Jellyfin path)** (`rpc.rs:3520-3560` `sync_needs_provider_routing` + `handle_sync_calculate_delta`) (AC: 6, 8)
  - [x] In `handle_sync_calculate_delta`, before choosing the Jellyfin-client fast path, check whether the selected server (or any resolved auto-fill descriptor's server) has a configured **non-default** pipeline (`pipeline_for(id).is_some_and(needs_configurable_expansion)`); if so, route through the provider path (`multi_provider_calculate_delta` / `provider_calculate_delta`) so the configurable engine runs. The Jellyfin-direct `run_auto_fill` block (`rpc.rs:4117-4138`) stays **unchanged** and is reached only for the pure-default case.
  - [x] Do not break the existing routing for non-auto-fill or default-auto-fill baskets — extend the decision, don't replace it.

- [x] **Tests** (`auto_fill` tests for `fetch`/discriminator; `rpc::` tests for routing) (AC: 10)
  - [x] `needs_configurable_expansion`: `default()`, `default_legacy(Some(n))` ⇒ false; a pipeline with a `Playlist` source / non-empty `filter` / `share` / `Unit::Album` / `[Quality]` ordering / non-empty `fallback` ⇒ true.
...
- `rtk cargo test -p hifimule-daemon` → 500 passed (was 488 pre-12.4; +12 new tests). Full suite ran clean in-sandbox (no mockito/networking gating hit this run).
- `rtk cargo clippy -p hifimule-daemon --all-targets` → no new warnings in touched modules (`auto_fill/fetch.rs`, `rpc.rs`, `device/mod.rs`). Remaining warnings (`vault.rs`, `api.rs::rename_item`, `device_io.rs`, `mtp.rs`, the two pre-existing `await_holding_lock` in `rpc.rs` tests) are all pre-existing and untouched.

### Implementation Plan

1. **`auto_fill/fetch.rs` (new)** — the async materialization half of the fetch/selection split.
   - `needs_configurable_expansion(&AutoFillPipeline) -> bool`: conservative default-legacy-equivalence check (sources, filter, ordering, unit, memory, fallback). `enabled`/`budget` excluded by design — budget is honored by the fast path's `max_fill_bytes`.
   - `expand_with_pipeline(provider, pipeline, params)`: clones+normalizes the pipeline (drop tags always; capability-gate genres via `BrowseMode::Genres` + runtime `UnsupportedCapability` → pass-through; cap `budget.max_bytes` by `params.max_fill_bytes`), materializes one pool per distinct `(kind, ref)` across `sources ∪ fallback` (Library paginated via `list_all_songs_page`; Favorites/History via priority-list calls; Playlist via `get_playlist` with blank-ref/NotFound/Unsupported → empty pool), attaches genre membership to each `Candidate`, then runs the pure `run_pipeline` with an empty `HistorySnapshot` (no DB read). Constants (`MAX_PER_LIST=2000`, `PAGE_SIZE=500`, `MAX_BULK_PAGES=200`) mirror `run_auto_fill_provider` verbatim.
2. **`auto_fill/mod.rs`** — `pub mod fetch; pub use fetch::*;`.
3. **`rpc.rs`** — single shared `expand_auto_fill_slot(provider, pipeline_opt, params)` seam (configurable when non-default, else `run_auto_fill_provider`); wired into both `multi_provider_calculate_delta`'s per-slot loop (reads `pipeline_for(&af_server)`) and `provider_calculate_delta`'s single-server block (reads the selected portable id's pipeline). New pure helper `auto_fill_needs_configurable_routing(manifest, auto_fill_servers)` extends `handle_sync_calculate_delta`'s routing decision so a configured non-default pipeline forces the per-provider path off the Jellyfin-client fast path.
4. **`device/mod.rs`** — removed `#[allow(dead_code)]` on `pipeline_for` (now referenced).

### Completion Notes List

- The pure engine (`pipeline.rs`) was reused verbatim — 12.4 is strictly the fetch/wiring layer feeding it. The pure-core tests (12.1) were not duplicated.
- Genre capability gating is belt-and-suspenders: a missing `BrowseMode::Genres` AND a runtime `UnsupportedCapability` from `get_genre_tracks` both drop genre constraints to pass-through (never emptying the fill).
- The `expand_auto_fill_slot` seam keeps the configurable-vs-default decision in exactly one place; the default path (`run_auto_fill_provider` / Jellyfin `run_auto_fill`) is byte-for-byte unchanged (AC 8) — only an explicitly non-default pipeline diverts.
- Scope boundaries honored: tag filter is config-only (dropped + logged), history is an empty snapshot (no `autofill_history` read/write), no headroom/duration/fallback-chain budget refinements, no UI/RPC-contract/schema/new-dependency changes.
- 12 new tests added: 2 discriminator (both branches) + 8 `expand_with_pipeline` (playlist materialization, missing-ref skip, genre include/exclude, genre capability-drop, tag-drop, share blend, budget cap, empty-history inertness, unsupported-source best-effort) in `fetch.rs`, + 1 routing-decision test in `rpc.rs`.

### File List

- `hifimule-daemon/src/auto_fill/fetch.rs` (new) — async pool-materialization layer (`expand_with_pipeline`, `needs_configurable_expansion`, fetch helpers) + tests.
- `hifimule-daemon/src/auto_fill/mod.rs` (modified) — declared `pub mod fetch; pub use fetch::*;`.
- `hifimule-daemon/src/rpc.rs` (modified) — `expand_auto_fill_slot` seam; `auto_fill_needs_configurable_routing` helper; wired both provider expansion sites; extended routing decision; routing test.
- `hifimule-daemon/src/device/mod.rs` (modified) — removed `#[allow(dead_code)]` on `pipeline_for`.

## Change Log

- 2026-06-14 — Story 12.4 dev complete (status → review). Built the async pool-materialization + `run_pipeline` fetch layer (`auto_fill/fetch.rs`): first-class `PlaylistSource` (`get_playlist`), capability-gated genre filter (`get_genre_tracks`/`BrowseMode::Genres`, pass-through fallback), config-only tag filter (dropped + logged), per-source share blending over real provider data, empty-`HistorySnapshot` inertness, and the `needs_configurable_expansion` default-vs-configurable discriminator. Wired `manifest.auto_fill.pipeline_for(serverId)` at the two provider sync-time expansion sites via a shared `expand_auto_fill_slot` seam, and forced provider routing (off the Jellyfin-client fast path) when a configured non-default pipeline applies. Single-server default behavior byte-for-byte unchanged; no UI/RPC-contract/schema/new-deps. `rtk cargo test -p hifimule-daemon` → 500 passed (+12); clippy clean in touched modules.
