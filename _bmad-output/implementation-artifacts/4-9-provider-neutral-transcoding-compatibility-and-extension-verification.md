...
- [ ] Task 1: Define profile compatibility checks in the sync path (AC: 2, 4, 5)
  - [ ] Derive the target device-compatible container/codec set from the active transcoding profile or device-preferred audio container.
  - [ ] Derive the source container/content type from provider metadata (`provider_suffix`, `provider_content_type`) before writing.
  - [ ] Treat profile compatibility as a hard device constraint, not a best-effort preference.

- [ ] Task 2: Enforce skip-on-unsupported-transcode behavior (AC: 2, 3, 5)
  - [ ] If required transcoding cannot be negotiated, record a warning/result entry and continue with the next item.
  - [ ] Do not write the file to the device.
  - [ ] Do not append skipped items to the manifest.
  - [ ] Ensure the UI-visible sync operation status can surface skipped/incompatible items clearly.

- [ ] Task 3: Preserve direct/passthrough correctness (AC: 4, 5)
  - [ ] If passthrough is compatible, keep the source suffix as the filename extension.
  - [ ] If passthrough is incompatible with the device profile, skip instead of writing.
  - [ ] Ensure filename extension is chosen from actual confirmed output content, not just the requested profile.

- [ ] Task 4: Keep provider boundaries intact (AC: 1, 6, 7)
  - [ ] For Subsonic/OpenSubsonic, call `provider.download_url(track_id, profile)` and keep stream URL construction inside `providers/subsonic.rs`.
  - [ ] For Jellyfin, preserve PlaybackInfo-based stream resolution and direct-play detection.
  - [ ] Avoid UI-specific or provider-specific branching in the generic sync engine except through provider/domain contracts.

- [ ] Task 5: Verification (AC: 1-7)
  - [ ] Add tests for Subsonic FLAC-to-MP3 stream URL generation with `maxBitRate` in kbps.
  - [ ] Add tests for compatible direct-download fallback preserving `.flac` or the actual source suffix.
  - [ ] Add tests for skipped incompatible direct downloads.
  - [ ] Add tests for skipped tracks when required transcoding cannot be honored.
  - [ ] Add tests proving skipped items are excluded from the manifest.
