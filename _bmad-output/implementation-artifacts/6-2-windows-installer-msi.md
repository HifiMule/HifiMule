...
so that I can install HifiMule like any other desktop application on my Windows PC, with the daemon auto-launching on login with full tray icon support.

## Acceptance Criteria

1. **MSI installs to Program Files**: Given a successful `cargo tauri build` on Windows, when I run the generated MSI, then HifiMule is installed to `C:\Program Files\HifiMule\` with Start Menu shortcuts.
2. **Daemon sidecar co-located**: The daemon sidecar (`hifimule-daemon.exe`) is placed alongside the main executable in the installation directory.
...
4. **Start Menu shortcuts functional**: Start Menu shortcut launches HifiMule correctly (UI starts, daemon sidecar spawns).
5. **App metadata correct**: The installer shows correct product name ("HifiMule"), manufacturer, version, and icon in Add/Remove Programs.
6. **Daemon registered as startup application**: Given the MSI installation completes, when the installer registers the startup entry, then `hifimule-daemon` is registered via a Registry `Run` key (`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`). The daemon launches automatically when the user logs in, running in the user session with full tray icon and notification support.
...
- [x] **T1: Validate Current WiX MSI Output** (AC: #1, #2, #5)
  ...
  - [x] T1.2: Install the MSI and verify installation directory is `C:\Program Files\HifiMule\`
  - [x] T1.3: Verify `hifimule-daemon.exe` is present alongside `HifiMule.exe` in the install directory
...
- [x] **T5: Register Daemon as Startup Application** (AC: #6, #8)
  - [x] T5.1: Create a WiX fragment that writes a Registry `Run` key (`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`) pointing to `hifimule-daemon.exe` in the install directory
  - [x] T5.2: Use WiX `<RegistryValue>` element with `Type="string"` and `Value="[INSTALLDIR]hifimule-daemon.exe"` — no custom actions needed
  ...
  - [x] T5.5: Verify after MSI install that `HKCU\...\Run\HifiMule` points to the correct daemon exe path
    ...
    <ComponentGroup Id="StartupRegistryGroup">
      <Component Id="StartupRegistryComponent"
        ...
        <RegistryValue
          ...
          Name="HifiMule"
          Type="string"
          Value="[INSTALLDIR]hifimule-daemon.exe"
          KeyPath="yes" />
      </Component>
    </ComponentGroup>
  </Fragment>
...
- Sidecar staging: `hifimule-ui/src-tauri/sidecars/`
- WiX fragments: `hifimule-ui/src-tauri/wix/` (replace `service-fragment.wxs` with startup registry fragment)
- App data (runtime): `%APPDATA%/HifiMule/` (daemon.log, ui.log, hifimule.db)
...
| File | Change |
|------|--------|
| `hifimule-ui/src-tauri/wix/service-fragment.wxs` | Replace with startup registry fragment (rename to `startup-fragment.wxs`) |
| `hifimule-ui/src-tauri/tauri.conf.json` | Update `fragmentPaths` and `componentGroupRefs` to reference new fragment |
| `hifimule-ui/src-tauri/src/lib.rs` | Update `SidecarStatus` default label from `"service"` to `"startup"` for health-check-detected daemon |
...
- [Source: hifimule-ui/src-tauri/tauri.conf.json] — Current Tauri configuration
- [Source: hifimule-ui/src-tauri/wix/service-fragment.wxs] — Current WiX service fragment (to be replaced)
- [Source: hifimule-ui/src-tauri/src/lib.rs] — Current 3-tier daemon detection logic
...
- **T5 (MSI)**: Created `wix/startup-fragment.wxs` with declarative `<RegistryValue>` targeting `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\HifiMule`. Uses GUID `1ba6130d-4fab-4dd0-b5cb-35477b1b5f4e` with `KeyPath="yes"` for automatic cleanup on uninstall. Updated `tauri.conf.json` to reference `startup-fragment.wxs` and `StartupRegistryGroup` (replacing old `service-fragment.wxs` and `ServiceComponents`).
- **T5 (NSIS)**: Created `nsis/hooks.nsh` with `NSIS_HOOK_POSTINSTALL` macro that writes the same `HKCU\...\Run\HifiMule` registry key via `WriteRegStr`, and `NSIS_HOOK_PREUNINSTALL` macro that removes it via `DeleteRegValue` on uninstall. Configured `tauri.conf.json` `nsis.installerHooks` to load the hook file.
- **Daemon icon**: Added `winresource` build dependency and `build.rs` to `hifimule-daemon` to embed `icon.ico` into the daemon executable. The startup application entry in Windows now shows the HifiMule icon.
...
- **T1-T4**: `cargo tauri build` produces `HifiMule_0.1.0_x64_en-US.msi` (5.2 MB). WiX compilation and linking succeeded. T1-T4 are manual validation tasks requiring MSI install/uninstall on Windows — existing Tauri v2 WiX configuration (UpgradeCode, InstallScope, shortcuts) inherited from story 6.1 is unchanged.
...
- `hifimule-ui/src-tauri/wix/startup-fragment.wxs` (new) — WiX fragment for HKCU Run key registration
- `hifimule-ui/src-tauri/wix/service-fragment.wxs` (deleted) — Removed orphaned WiX service fragment (replaced by startup-fragment.wxs)
- `hifimule-ui/src-tauri/nsis/hooks.nsh` (new) — NSIS installer hooks for HKCU Run key registration (install + uninstall)
- `hifimule-ui/src-tauri/tauri.conf.json` (modified) — Updated fragmentPaths, componentGroupRefs, and added nsis.installerHooks
- `hifimule-ui/src-tauri/Cargo.toml` (modified) — Added "blocking" feature to reqwest for synchronous daemon health check
- `hifimule-ui/src-tauri/src/lib.rs` (modified) — Updated SidecarStatus from "service" to "startup" for health-check detection
- `hifimule-daemon/Cargo.toml` (modified) — Added winresource build dependency, windows-service dependency
- `hifimule-daemon/build.rs` (new) — Embeds icon.ico into the daemon executable on Windows
- `hifimule-daemon/src/main.rs` (modified) — Refactored to extract `start_daemon_core()`, added service flag parsing, log truncation
- `hifimule-daemon/src/service.rs` (new) — Windows Service infrastructure (install/uninstall/run) for power users
- `hifimule-daemon/src/tests.rs` (modified) — Added `test_start_daemon_core_returns_shutdown_and_receiver` test
