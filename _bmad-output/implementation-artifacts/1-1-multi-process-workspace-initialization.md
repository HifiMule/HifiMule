...
1. **Given** a clean project directory, **When** I run `cargo build`, **Then** the workspace successfully compiles both `jellyfinsync-daemon` and `jellyfinsync-ui` (Tauri).
2. **And** `jellyfinsync-daemon` starts as a standalone headless binary.

## Tasks / Subtasks

- [x] Task 1: Initialize Root Cargo Workspace (AC: 1)
  - [x] Create root `Cargo.toml` defining the workspace members (`jellyfinsync-daemon`, `jellyfinsync-ui`).
  - [x] Configure `.gitignore` to exclude `target/` and node dependencies.
- [x] Task 2: Create `jellyfinsync-daemon` Crate (AC: 2)
  - [x] Initialize `jellyfinsync-daemon` as a binary crate.
...
- [x] Task 3: Create `jellyfinsync-ui` Tauri App (AC: 1)
  - [x] Initialize `jellyfinsync-ui` using `create-tauri-app` (Tauri v2).
...
- [x] Task 4: Verify Workspace Build (AC: 1)
  ...
  - [x] Verify `jellyfinsync-daemon` binary is produced in `target/debug`.
...
- **Workspace Layout:**
  ...
  ├── jellyfinsync-daemon/   # Rust binary crate
  │   ├── Cargo.toml
  │   └── src/
  └── jellyfinsync-ui/       # Tauri app
      ├── src-tauri/      # Rust Tauri backend
      ├── src/            # TypeScript frontend
      └── package.json
  ```
- **Naming Conventions:** 
  - Crates: `kebab-case` (`jellyfinsync-daemon`)
...
- Created root `Cargo.toml` workspace with members `jellyfinsync-daemon` and `jellyfinsync-ui/src-tauri`
...
- Created `jellyfinsync-daemon` binary crate with:
...
- Initialized `jellyfinsync-ui` Tauri v2 app with vanilla-ts template using `create-tauri-app`
...
- Successfully ran `npm install` in jellyfinsync-ui directory (19 packages installed)
...
- Verified binaries produced:
  - `target/debug/jellyfinsync-daemon.exe` (937 KB)
  - `target/debug/jellyfinsync-ui.exe` (12.9 MB)
- All tests passed: 2/2 in jellyfinsync-daemon (test_daemon_compiles, test_tokio_runtime_works)
- All acceptance criteria verified:
  - AC 1: Both `jellyfinsync-daemon` and `jellyfinsync-ui` compile successfully ✅
  - AC 2: `jellyfinsync-daemon` starts as standalone headless binary (verified via tests) ✅
...
- **Fix Applied:** Added missing "Shoelace" Web Components CDN links to `jellyfinsync-ui/index.html`.
- **Fix Applied:** Removed `Cargo.lock` from `.gitignore` to ensure reproducible builds.
- **Fix Applied:** Updated `jellyfinsync-ui` crate metadata (description, authors).
...
- `jellyfinsync-ui/src-tauri/Cargo.toml`
- `.gitignore`
- `target/debug/jellyfinsync-daemon.exe` (build artifact)
- `target/debug/jellyfinsync-ui.exe` (build artifact)
