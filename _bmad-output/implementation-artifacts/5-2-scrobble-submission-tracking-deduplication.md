...
**Files to MODIFY:**
1. [jellysync-daemon/src/db.rs](jellysync-daemon/src/db.rs) — Add `is_scrobble_recorded()` method + 3 unit tests
2. [jellysync-daemon/src/scrobbler.rs](jellysync-daemon/src/scrobbler.rs) — Add `skipped_duplicate` field to `ScrobblerResult`, add dedup check in `process_device_scrobbles()`, update invariant test, add dedup scenario test
...
- **Cargo test target**: `cargo test` in `jellysync-daemon/` — all 91 existing tests + 4 new = 95 total.
...
- The dedup check fits naturally in the existing processing loop at [jellysync-daemon/src/scrobbler.rs:128](jellysync-daemon/src/scrobbler.rs#L128).
...
- `jellysync-daemon/src/api.rs`, `scrobbler.rs`, `sync.rs` — modified. These are the Story 5.1 review fixes (not yet committed at story start). Do NOT revert any of these changes.
...
- [jellysync-daemon/src/db.rs:97](jellysync-daemon/src/db.rs#L97) — `record_scrobble()` method — `is_scrobble_recorded()` goes directly below it
- [jellysync-daemon/src/db.rs:115](jellysync-daemon/src/db.rs#L115) — `get_scrobble_count()` — pattern reference for `is_scrobble_recorded()`
- [jellysync-daemon/src/db.rs:68](jellysync-daemon/src/db.rs#L68) — `idx_scrobble_unique` index definition — used by the dedup query
- [jellysync-daemon/src/scrobbler.rs:19](jellysync-daemon/src/scrobbler.rs#L19) — `ScrobblerResult` struct — add `skipped_duplicate` field here
- [jellysync-daemon/src/scrobbler.rs:122](jellysync-daemon/src/scrobbler.rs#L122) — Counter initialization block — add `let mut skipped_duplicate = 0usize;` here
- [jellysync-daemon/src/scrobbler.rs:128](jellysync-daemon/src/scrobbler.rs#L128) — Top of the entry processing loop — dedup check goes after line 131 (the `rating != "L"` gate)
- [jellysync-daemon/src/scrobbler.rs:214](jellysync-daemon/src/scrobbler.rs#L214) — Final `ScrobblerResult { ... }` construction — add `skipped_duplicate` field here
- [jellysync-daemon/src/scrobbler.rs:339](jellysync-daemon/src/scrobbler.rs#L339) — `test_scrobbler_result_submitted_excludes_db_failures` — update struct literal and assertion
