...
*   **The Sprinter (Sarah):** Needs zero friction. Auto-Fill + Auto-Sync on Connect is her primary path — plug in, walk away. She can now define a distinct fill per server (e.g. home Jellyfin vs. work Navidrome).
...
### 5.3 Auto-Fill Configuration & Slots
*   **Auto-Fill Configuration Panel:** Opened from the Basket header (scoped to the currently selected server). A **pipeline builder** with collapsible stage sections — **Filter** (genre/tag include-exclude chips) → **Sources** (add Playlist pools / Library / Favorites / History, each with a share slider) → **Unit** (track/album/artist segmented control) → **Ordering** (drag-to-reorder ranking keys) → **Memory** (cooldown weeks, repeat-tolerance dial, stable-core %) → **Budget** (size or duration target + headroom reserve). Advanced stages (Memory beyond cooldown, and the quality/discovery/delight modifiers) appear progressively behind an "Advanced" disclosure so the simple path stays one-click. A **"Default"** state (no config) renders the simplest form and behaves exactly like the legacy favorites → play count → creation date fill. Each smart strategy surfaces its cheap playlist/tag-based equivalent inline (ambition tiers).
*   **Per-Server Auto-Fill:** Configuration is scoped to the currently selected server; switching servers shows that server's config. Each server's enabled auto-fill produces its own slot card in the basket — multiple coexist.
*   **Auto-Fill Slot Card (per server):** A dashed-border card carrying the server's icon + name badge: "Auto-Fill · {server} · will fill ~X GB / ~Y h at sync time". When a duration target is set, the card shows the derived bytes; when sources can't reach the target, a subtle "fallback fills the rest" hint is shown. Cards for non-selected servers render **read-locked** (no toggle/edit affordance), consistent with the existing locked-basket-item pattern. No API call is made when Auto-Fill is toggled or reconfigured — slots are local UI markers; the pipeline persists via `autoFill.setPipeline`.
