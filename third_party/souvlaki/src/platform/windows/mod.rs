...
impl MediaControls {
        ...
        F: Fn(MediaControlEvent) -> bool + Send + 'static,
    {
        ...
        let button_handler = TypedEventHandler::new({
            ...
            move |_, args: &Option<_>| {
                ...
                let _ = (event_handler.lock().unwrap())(event);
                Ok(())
            }
        });
        ...
        self.position_handler_token = Some(
            self.controls
    ...
    pub fn set_capabilities(
        ...
        self.controls.SetIsPlayEnabled(capabilities.play)?;
        self.controls.SetIsPauseEnabled(capabilities.pause)?;
        self.controls.SetIsStopEnabled(capabilities.stop)?;
        self.controls.SetIsNextEnabled(capabilities.next)?;
        self.controls.SetIsPreviousEnabled(capabilities.previous)?;
        self.controls.SetIsFastForwardEnabled(capabilities.seek)?;
        self.controls.SetIsRewindEnabled(capabilities.seek)?;
        Ok(())
    }
    ...
    pub fn detach(&mut self) -> Result<(), Error> {
        ...
        if let Some(position_handler_token) = self.position_handler_token {
            self.controls
                .RemovePlaybackPositionChangeRequested(position_handler_token)?;
            self.position_handler_token = None;
        }
    ...
    pub fn set_metadata(&mut self, metadata: MediaMetadata) -> Result<(), Error> {
        // Metadata is a replacement. Clear fields and artwork retained by a
        // prior track before publishing this item.
        self.display_updater.ClearAll()?;
        self.display_updater.SetType(MediaPlaybackType::Music)?;
