...
use crate::{
    ...
    OutputCallbackInfo, OutputStreamTimestamp, PauseSnapshot, ResultExt, SampleFormat, SampleRate,
...
pub enum Command {
    PlayStream,
    PauseStream(Option<SyncSender<Result<PauseSnapshot, Error>>>),
    Terminate,
}
...
impl StreamTrait for Stream {
    ...
    fn pause(&self) -> Result<(), Error> {
        self.push_command(Command::PauseStream(None))
    }

    fn pause_with_snapshot(&self) -> Result<PauseSnapshot, Error> {
        let (reply, result) = sync_channel(1);
        self.push_command(Command::PauseStream(Some(reply)))?;
        result.recv().map_err(|_| {
            Error::with_message(
                ErrorKind::StreamInvalidated,
                "acknowledged pause response channel closed",
            )
        })?
    }
...
fn process_commands(run_context: &mut RunContext) -> Result<bool, Error> {
    ...
    for command in run_context.commands.try_iter() {
        match command {
            ...
            Command::PauseStream(reply) => {
                if let Some(reply) = reply {
                    let _ = reply.send(pause_snapshot(&mut run_context.stream));
                } else {
                    unsafe {
                        if run_context.stream.playing {
                            run_context
...
fn pause_snapshot(stream: &mut StreamInner) -> Result<PauseSnapshot, Error> {
    unsafe {
        if !matches!(&stream.client_flow, AudioClientFlow::Render { .. }) {
            return Err(Error::with_message(
                ErrorKind::UnsupportedOperation,
                "presentation snapshots are only available for output streams",
            ));
        }
        if stream.playing {
            stream
        ...
        stream
            ...
            .context("Failed to capture stopped audio position")?;
        let frequency = stream
            .audio_clock
            .GetFrequency()
            .context("Failed to capture audio clock frequency")?;
        if frequency == 0 {
            return Err(Error::with_message(
