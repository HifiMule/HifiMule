...
pub trait StreamTrait {
    ...
    /// Pause and wait until the backend acknowledges suspension, returning its
    /// presentation cutoff. Queued frames reported by the snapshot are flushed
    /// before return. Backends without a reliable cutoff return
    /// `UnsupportedOperation` without changing stream state.
    fn pause_with_snapshot(&self) -> Result<crate::PauseSnapshot, Error> {
        Err(Error::with_message(
            crate::ErrorKind::UnsupportedOperation,
            "acknowledged pause snapshot is unsupported by this backend",
        ))
    }
