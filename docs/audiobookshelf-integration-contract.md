...
## Record 3 — grouping follow-up (2026-09-25)

The controlled server reported v2.36.1. Read-only probes with a disposable account observed `GET /api/libraries/{bookLibrary}/series?page={page}&limit=1` and the matching collections route. Both returned `results`, `total`, `limit`, and `page`; series had four entries over pages 0 and 1, while collections had one entry and an empty page 1. The series list entries carried ordered `books` arrays. `GET /api/series/{id}` returned only series metadata, without `books`; `GET /api/collections/{id}` returned books in the same order as the list. All observed members had the selected Books library ID and `book` media type. The collection overlapped several series, showing that grouping identity must use its type and upstream ID, never name or membership.

The series list did not expose `seriesSequence` on its member objects in this deployment. Preserve the array order returned by the source as an observed order, without claiming it is a separate sequence field. For collections, preserve the source `books` array order. A series detail request alone cannot reconstruct membership; use the paged list or a separately validated filtered item route. The redacted alias-only evidence is in `hifimule-daemon/tests/fixtures/audiobookshelf/2.36.1/grouping-observations.json`. No origin, credentials, real IDs, titles, media paths, or raw responses were retained. Stale and cross-library member behavior were not present in the controlled corpus and require offline fault fixtures.

## Record 4 — direct-media seeking qualification (2026-09-25)

The verified v2.36.1 direct delivery contract above covers book-part MP3 and AAC/MP4, and the podcast playback adapter accepts the same direct formats after an authenticated range probe. HifiMule now offers only those direct representations as seek candidates. The candidate remains unavailable until the existing player verifies the opened FFmpeg container and codec, reconciles positive item duration, and confirms the HTTP range representation. The player measures decoded landing within 50 ms before committing a seek. A rejected or changed range, duration mismatch, or unsupported format cannot turn into an advertised seek capability. HLS/transcode remains unsupported.

Offline generated MP3 and AAC/MP4 fixtures exercise forward, backward, and repeated seeks with independent sample correlation. These tests establish decoder behavior for those formats; they do not establish an installed-app or controlled-server seek result for Audiobookshelf. The controlled deployment's authenticated range observations establish transport shape only. Installed playback and seek evidence remains to be collected separately.
