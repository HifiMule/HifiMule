...
| Search | `GET /api/libraries/{library}/search?q=...` returned a role-specific empty result shape: Books exposed `book`, `authors`, `narrators`, `series`, `genres`, `tags`; Podcasts exposed `podcast`, `episodes`, `genres`, `tags`. | Never force podcast results through the Books shape. Search paging remains **ambiguous** because the observed route has no paging fields. |
...
| Transcode | verified for playback only | The v2.36.1 source (commit `e874d31e646b20d62559274b55bf42bc511d3f46`) defines `forceTranscode`; a controlled `forceTranscode: true` session returned `playMethod: 2` and one server-relative playback track. This is session playback, not an item-download conversion capability. |
...
`audiobookshelf_contract.rs` is entirely offline. It validates schema version, redaction policy, required matrix cases, pagination/order/identity invariants, and an HTTP request’s method, path, paging query, and bearer-header shape through `mockito`. It uses only `[fixture-token]`, never a secret.
